...
class ElectrolyteGPFitter(gp_core.GPFitter):
  def __init__(self, X, Y, options=None, reporter=None):
    ...
    if options is None:
      options = load_options(electrolyte_gp_args,
                             "ElectrolyteGPFitter",
                             reporter=reporter)
  ...
  def _child_set_up(self):
    ...
    self.cts_hp_bounds += [self.scale_log_bounds]
    # bandwidth hyper-parameter
    ...
    self.cts_hp_bounds += self.bandwidth_log_bounds
  ...
  def _child_build_gp(self, mean_func, noise_var, gp_cts_hps, gp_dscr_hps,
    ...
    # scale, bandwidth(s), (nu) removed from cont/discrete hyper parameters
    electrolyte_kernel = ElectrolyteKernel(nu=matern_nu,
                                           scale=scale,
                                           dim_bandwidths=ke_dim_bandwidths)
    # generate gp from kernel
    ret_gp = ElectrolyteGP(self.X, self.Y, electrolyte_kernel,
...
class ElectrolyteGP(gp_core.GP):
  def __init__(self, X, Y, kernel, mean_func, noise_var,
    ...
    super(ElectrolyteGP, self).__init__(X, Y, kernel, mean_func,
                                        noise_var, build_posterior, reporter)

  def _child_str(self):
    """ String representation for child GP. """
    ke_str = "electrolyte(nu={:.1f})".format(self.kernel.hyperparams["nu"])
    ke_str = ke_str + \
             get_list_of_floats_as_str(self.kernel.hyperparams["dim_bandwidths"])
